...
open class SearchTextField: UITextField ,UITextFieldDelegate{
    ...
    open func filterStrings(_ strings: [String]) {
        ...
        for value in strings {
            items.append(SearchTextFieldItem(id: nil, title: value))
        }
    ...
    @objc open func textFieldDidBeginEditing() {
        if (startVisible || startVisibleWithoutInteraction) && text!.isEmpty {
            clearResults()
            filter(forceShowAll: true)
            self.placeholderLbl.textColor = UIColor.darkGray
            
        }
        placeholderLabel?.attributedText = nil
        
        
        usernameLabelYAnchorConstraint.constant = -20
        ...
        self.placeholderLbl.textColor = ColorPalette.mainColor
    }
    ...
    @objc open func textFieldDidEndEditing() {
        ...
        if let text = self.text, text.isEmpty {
            ...
            performAnimation(transform: CGAffineTransform(scaleX: 1, y: 1))
            self.placeholderLbl.font = Fonts.SegoUI.regular(of: 15)
            self.placeholderLbl.textColor = UIColor.lightGray
        }
    }
